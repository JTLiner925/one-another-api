...
usersRouter.route("/signup").post((req, res, next) => {
  // res.send("Hello, node!");
  const knexInstance = req.app.get("db");
  const { user_address, user_bio, user_email, user_password, first_name, last_name } = req.body;
  bcrypt.hash(user_password, 12).then((hashedPassword) => {

  
  let userData = { user_address, user_bio, user_email, user_password: hashedPassword, first_name, last_name}
